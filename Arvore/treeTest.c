...
int main(int argc, char **argv) {
    ...
    assert(strcmp(el->value, "Val 50") == 0);findEl.key = 53;
    assert(find(tree, &findEl, &BinarySearchTreeElementComparator,(void **) &el) == 1);
    assert(strcmp(el->value, "Val 53") == 0);findEl.key = 45;
    assert(find(tree, &findEl, &BinarySearchTreeElementComparator,(void **) &el) == 1);
    assert(strcmp(el->value, "Val 45") == 0);findEl.key = 60;
    assert(find(tree, &findEl, &BinarySearchTreeElementComparator,(void **) &el) == 1);
    assert(strcmp(el->value, "Val 60") == 0);
    // Tentativa que não encontra na árvore
    findEl.key = 10;
    assert(find(tree, &findEl, &BinarySearchTreeElementComparator,(void **) &el) == 0);
    el = (BinarySearchTreeElement *) malloc(sizeof(BinarySearchTreeElement));
    el->key=45;
    removeTreeNode(&tree, el, &BinarySearchTreeElementComparator);
    in_order(tree, printInteger);//ESQUERDA RAIZ DIREITA
    printf("\n");
    pre_order(tree, printInteger);//ESQUERDA DIREITA RAIZ
    printf("\n");
    post_order(tree, printInteger);//RAIZ ESQUERDA DIREITA
    printf("\n");
